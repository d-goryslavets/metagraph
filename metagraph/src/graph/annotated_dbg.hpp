...
class AnnotatedDBG : public AnnotatedSequenceGraph {
  public:
     ...
     */
    std::vector<std::vector<std::tuple<std::string, Label, uint64_t, uint64_t>>>
    get_overlapping_reads(const std::vector<node_index> &nodes, std::unordered_set<std::string> manifest_labels, bool auto_labels = false, const char query_identifier = 'A') const;
